...
{
    public partial class AdminFuelLogListViewModel : BaseViewModel
    {
        ...
        public AdminFuelLogListViewModel(IAuthService auth, INavigationService nav, IFuelLogService fuelLogService)
            : base(auth, nav)
        {
        ...
        [RelayCommand]
        private Task AddAsync() => Nav.GoToAsync("AdminFuelLogForm");

        [RelayCommand]
        private Task EditAsync(FuelLogItem item)
        {
            if (item == null) return Task.CompletedTask;
            return Nav.GoToAsync("AdminFuelLogForm", new Dictionary<string, object> { { "FuelLogItem", item } });
        }
    }
}
