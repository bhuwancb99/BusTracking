namespace BusTracking.Mobile.Viewmodels.Coordinator
{
    ...
    public partial class CoordTeacherFormViewModel : BaseViewModel
    {
        ...
        public CoordTeacherFormViewModel(IAuthService auth, INavigationService nav, ITeacherService teacherService)
            : base(auth, nav)
        {
        ...
        private async Task LoadTeacherDetailsAsync(int id)
        {
            ...
            {
                var t = await _teacherService.GetTeacherByIdAsync(id, isCoordinator: true);
                if (t != null)
                {
                    PopulateFields(t);
                }
        ...
        private async Task SaveAsync()
        {
            ...
            await RunAsync(async () =>
            {
                if (IsEditMode)
                {
                    ...
                    var res = await _teacherService.UpdateTeacherAsync(TeacherId, req, isCoordinator: true);
                    if (res.Success)
                    {
                ...
                else
                {
                    ...
                    var res = await _teacherService.CreateTeacherAsync(req, isCoordinator: true);
                    if (res.Success)
                    {
                        await Nav.GoBackAsync();
                    }
