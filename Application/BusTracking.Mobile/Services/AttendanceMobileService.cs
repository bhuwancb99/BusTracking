namespace BusTracking.Mobile.Services
{
    public class AttendanceMobileService : IAttendanceMobileService
    {
        ...
        {
            var url = $"api/teacher/attendance/students?academicYearId={academicYearId}&standardId={standardId}&date={date:yyyy-MM-dd}";
        ...
        {
            var res = await _api.PostAsync<bool>("api/teacher/attendance/manual-batch", dto);
        ...
        {
            var res = await _api.PostAsync<FaceAttendanceScanResultDto>("api/teacher/attendance/face-scan", dto);
        ...
        {
            var url = $"api/teacher/attendance/report?academicYearId={academicYearId}&standardId={standardId}&date={date:yyyy-MM-dd}";
            if (sectionId.HasValue && sectionId.Value > 0) url += $"&sectionId={sectionId.Value}";
