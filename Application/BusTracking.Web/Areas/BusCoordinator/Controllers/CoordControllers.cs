...
public class BusController : Controller
{
    ...
    public BusController(IBusService b, IRouteService r) { _bus = b; _route = r; }
    ...
    public async Task<IActionResult> Index(int page = 1, string? search = null, string? status = "Active")
    {
        ...
        var r0 = await _bus.GetAllAsync(page, 10, search, status); return View(r0.Data);
    }

    [HttpGet]
    public async Task<IActionResult> Create()
    { await LoadRoutes(); return View(new CreateBusDto()); }
    ...
    public async Task<IActionResult> Create(CreateBusDto m)
    {
        ...
        if (!r.Success) { ModelState.AddModelError("", r.Message); await LoadRoutes(); return View(m); }
    ...
    public async Task<IActionResult> Edit(int id)
    {
        ...
        ViewBag.BusId = id; await LoadRoutes(r.Data!.RouteId);
        return View(new UpdateBusDto { BusName = r.Data.BusName, BusNumber = r.Data.BusNumber, RouteId = r.Data.RouteId, Capacity = r.Data.Capacity, IsActive = r.Data.IsActive });
    }

    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(int id, UpdateBusDto m)
    {
        if (!ModelState.IsValid) { ViewBag.BusId = id; await LoadRoutes(m.RouteId); return View(m); }
        var r = await _bus.UpdateAsync(id, m);
        if (!r.Success) { ModelState.AddModelError("", r.Message); ViewBag.BusId = id; await LoadRoutes(m.RouteId); return View(m); }
        TempData["SuccessMessage"] = r.Message; return RedirectToAction(nameof(Index));
    }
...
public class StudentController : Controller
{
    ...
    public async Task<IActionResult> Edit(int id)
    {
        var r = await _s.GetByIdAsync(id); if (!r.Success) return NotFound(); ViewBag.StudentId = id;
    ...
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> Delete(int id)
    { await _s.DeleteAsync(id); TempData["SuccessMessage"] = "Marked inactive."; return RedirectToAction(nameof(Index)); }

    [HttpPost]
    public async Task<IActionResult> Toggle(int id)
    { var r = await _s.ToggleActiveAsync(id); return Json(new { r.Success, r.Message }); }

    [HttpPost]
    public async Task<IActionResult> AssignBus([FromBody] BusTracking.Common.DTOs.Assign.AssignBusToStudentDto dto)
    { var r = await _s.AssignBusAsync(dto); return Json(new { r.Success, r.Message }); }
    ...
    { var r = await _s.SearchAsync(q); return Json(r.Data); }
    ...
    { var r = await _s.GetAvailabilitiesAsync(studentId); ViewBag.StudentId = studentId; return View(r.Data); }
...
// ── Driver ────────────────────────────────────────────────────
// CHANGE 3: Added — coordinators with driver.view/add/edit can now
//           reach Driver pages in the BusCoordinator area.
[Area("BusCoordinator"), Authorize(Roles = "BusCoordinator")]
...
// ── Parent ────────────────────────────────────────────────────
// CHANGE 3: Added — coordinators with parent.view/add/edit access
[Area("BusCoordinator"), Authorize(Roles = "BusCoordinator")]
public class ParentController : Controller
{
    ...
    private int UserId => int.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier) ?? "0");
    public ParentController(IParentService parent, IStudentService student) { _parent = parent; _student = student; }
...
// ── Route ─────────────────────────────────────────────────────
// CHANGE 3: Added — coordinators with route.view/add/edit access
[Area("BusCoordinator"), Authorize(Roles = "BusCoordinator")]
...
// ── Notification ──────────────────────────────────────────────
// CHANGE 3: Added — coordinators with notification.manage access
[Area("BusCoordinator"), Authorize(Roles = "BusCoordinator")]
